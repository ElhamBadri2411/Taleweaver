...
const generateAudio = async (req, res, next) => {
    ...
    try {
        ...
        const response = await axios.post(`https://texttospeech.googleapis.com/v1/text:synthesize?key=${apiKey}`, requestBody);
        ...
        await writeFile(audioPath, Buffer.from(response.data.audioContent, 'base64'));
